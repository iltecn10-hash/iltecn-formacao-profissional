...
export function NewSpreadsheetForm({ missions }: { missions: { id: string; title: string }[] }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setLoading(true);
    const selected = SPREADSHEET_TEMPLATES.find((t) => t.key === templateKey);
    const res = await fetch("/api/student-works", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        missionId,
        workType: "SPREADSHEET",
    ...
    setLoading(false);
    if (!res.ok) {
      setError(data.error ?? "Não foi possível criar a planilha.");
      return;
    }
    router.push(`/dashboard/trabalhos/planilha/${data.work.id}`);
  }

  if (missions.length === 0) {
    return (
      <p className="text-sm text-muted">
        Ainda não há missões cadastradas para vincular uma planilha.
      </p>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        ...
        <select
          ...
          className={inputClass}
        >
          <option value="">Planilha livre (sem modelo)</option>
